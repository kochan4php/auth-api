...
const register = async (req, res) => {
  try {
    ...
    Users.create({ name, email, password: hashPassword }, (err, user) => {
      return err
        ? res
            .status(400)
            .json({ message: "Gagal registrasi, silahkan coba kembali" })
        : res
            .status(201)
            .json({ message: "Registrasi berhasil, silahkan login", user });
    });
  } catch (error) {
    console.log(error);
  }
...
const login = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(404)
        .json({ message: "User tidak terdaftar, silahkan register!" });
...
const logout = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Berhasil logout, sampai jumpa :D" });
  } catch (error) {
    console.log(error);
  }
};
